...
TRANSLATEREQUESTTEXT_ONE_OF_SCHEMAS = ["List[str]", "str"]
...
class TranslateRequestText(BaseModel):
    ...
    """
    # data type: str
    oneof_schema_1_validator: Optional[StrictStr] = Field(default=None, description="Text content to translate")
    # data type: List[str]
    oneof_schema_2_validator: Optional[List[StrictStr]] = Field(default=None, description="Array of text snippets for batch translation. Using this method is recommended for translating multiple texts as it's faster and allows for context-aware translations across snippets.")
    ...
    def actual_instance_must_validate_oneof(cls, v):
        instance = TranslateRequestText.model_construct()
        error_messages = []
        match = 0
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when setting `actual_instance` in TranslateRequestText with oneOf schemas: List[str], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when setting `actual_instance` in TranslateRequestText with oneOf schemas: List[str], str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        if match > 1:
            # more than 1 match
            raise ValueError("Multiple matches found when deserializing the JSON string into TranslateRequestText with oneOf schemas: List[str], str. Details: " + ", ".join(error_messages))
        elif match == 0:
            # no match
            raise ValueError("No match found when deserializing the JSON string into TranslateRequestText with oneOf schemas: List[str], str. Details: " + ", ".join(error_messages))
        else:
            return instance

...
class TranslateDictionaryRequest(BaseModel):
    """
    TranslateDictionaryRequest
    """ # noqa: E501
    word: StrictStr = Field(description="The word to look up")
    word_language: Optional[StrictStr] = Field(default='en', description="Language code (ISO-639) of the word, or \"auto\" for automatic detection")
    definition_language: Optional[StrictStr] = Field(default='en', description="Language code (ISO-639) for the definition output")
    stream: Optional[StrictBool] = Field(default=False, description="Whether to return a streaming response with fields sent as they become available")
    nsfw: Optional[StrictBool] = Field(default=True, description="Whether to allow NSFW/inappropriate content. If false, returns empty definition for inappropriate words")
    model: Optional[StrictStr] = Field(default=None, description="Optional model identifier. Can be either a model name (e.g., \"gpt-4o\", \"claude-35-sonnet\", \"gemini-flash-2.0\") or a model constant (e.g., \"ANTHROPIC_CLAUDE_35_SONNET\", \"OPENAI_GPT4O\"). See model-selection.ts for full list.")
    __properties: ClassVar[List[str]] = ["word", "word_language", "definition_language", "stream", "nsfw", "model"]
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "word": obj.get("word"),
            "word_language": obj.get("word_language") if obj.get("word_language") is not None else 'en',
            "definition_language": obj.get("definition_language") if obj.get("definition_language") is not None else 'en',
            "stream": obj.get("stream") if obj.get("stream") is not None else False,
            "nsfw": obj.get("nsfw") if obj.get("nsfw") is not None else True,
            "model": obj.get("model")
        })
        return _obj
